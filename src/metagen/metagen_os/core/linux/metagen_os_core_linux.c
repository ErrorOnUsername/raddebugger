...
{
  if (flags & (OS_AccessFlags_Read | OS_AccessFlags_Write))
  {
    return "rwb";
  }
...
os_file_open(OS_AccessFlags flags, String8 path)
{
  ...
  String8 path_str = push_str8_copy(scratch.arena, path);
  file.u64[0] = (U64)fopen((char*)path.str, mode);
  scratch_end(scratch);
  return file;
}
...
os_file_iter_begin(Arena *arena, String8 path, OS_FileIterFlags flags)
{
  ...
  // NOTE(rd): this is scuffed, but it's easier than read from /proc/self/fd
...
os_file_iter_next(Arena *arena, OS_FileIter *iter, OS_FileInfo *info_out)
{
  ...
  {
    dirent *dir_ent = readdir(lnx_iter->dir);
    while (dir_ent != NULL)
    {
      B32 usable_file = 1;
